...
#define	RP1_ETH_MBOX_COMPAT	"brcm,bcm2835-mbox"
#define	RP1_ETH_MBOX_READ	0x00
#define	RP1_ETH_MBOX_STATUS	0x18
#define	RP1_ETH_MBOX_WRITE	0x20
#define	RP1_ETH_MBOX_FULL	0x80000000
#define	RP1_ETH_MBOX_EMPTY	0x40000000
#define	RP1_ETH_MBOX_CHAN_PROP	8
#define	RP1_ETH_MBOX_RESP_OK	0x80000000
#define	RP1_ETH_MBOX_TAG_MAC	0x00010003
#define	RP1_ETH_MBOX_SPIN	1000000

struct rp1_eth_mbox_mac {
	uint32_t	buf_size;
	uint32_t	code;
	uint32_t	tag;
...
static bool
rp1_eth_mac_is_zero(const uint8_t mac[6])
{
	int i;
...
static void
rp1_eth_mbox_getaddr(void *arg, bus_dma_segment_t *segs, int nseg, int error)
{
...
/*
 * The board MAC lives in SoC OTP, readable only by the VideoCore.  Ask it over
 * the mailbox property channel; BCM2712 takes a flat physical buffer address.
 */
static int
rp1_eth_fw_get_macaddr(phandle_t root, uint8_t mac[6])
{
	struct rp1_eth_mbox_mac *msg;
	bus_dma_tag_t tag;
	bus_dmamap_t map;
	bus_addr_t phys;
	bus_space_tag_t bst;
	...
	node = rp1_eth_fdt_find_compat(root, RP1_ETH_MBOX_COMPAT);
	if (node == 0)
		return (ENODEV);

	phys = 0;
	err = bus_dma_tag_create(NULL, 16, 0, BUS_SPACE_MAXADDR_32BIT,
	    BUS_SPACE_MAXADDR, NULL, NULL, sizeof(*msg), 1, sizeof(*msg), 0,
	    NULL, NULL, &tag);
	if (err != 0)
		return (err);
	err = bus_dmamem_alloc(tag, (void **)&msg,
	    BUS_DMA_COHERENT | BUS_DMA_WAITOK | BUS_DMA_ZERO, &map);
	if (err != 0) {
		bus_dma_tag_destroy(tag);
		return (err);
	}
	err = bus_dmamap_load(tag, map, msg, sizeof(*msg),
	    rp1_eth_mbox_getaddr, &phys, BUS_DMA_NOWAIT);
	if (err != 0 || phys == 0) {
		err = (err != 0) ? err : ENXIO;
		goto free;
	}

	err = OF_decode_addr(node, 0, &bst, &bsh, &bsz);
	if (err != 0)
		goto unload;

	msg->buf_size = sizeof(*msg);
	msg->code = 0;
	msg->tag = RP1_ETH_MBOX_TAG_MAC;
	msg->val_buf_size = sizeof(msg->mac);
	msg->val_len = 0;
	msg->end_tag = 0;
	bus_dmamap_sync(tag, map, BUS_DMASYNC_PREWRITE | BUS_DMASYNC_PREREAD);

	for (i = 0; i < RP1_ETH_MBOX_SPIN; i++) {
		if ((bus_space_read_4(bst, bsh, RP1_ETH_MBOX_STATUS) &
		    RP1_ETH_MBOX_FULL) == 0)
			break;
		DELAY(1);
	}
	...
	bus_space_write_4(bst, bsh, RP1_ETH_MBOX_WRITE,
	    ((uint32_t)phys & ~0xfU) | RP1_ETH_MBOX_CHAN_PROP);

	for (i = 0; i < RP1_ETH_MBOX_SPIN; i++) {
		if ((bus_space_read_4(bst, bsh, RP1_ETH_MBOX_STATUS) &
		    RP1_ETH_MBOX_EMPTY) == 0)
			break;
		DELAY(1);
	}
	if ((bus_space_read_4(bst, bsh, RP1_ETH_MBOX_STATUS) &
	...
	(void)bus_space_read_4(bst, bsh, RP1_ETH_MBOX_READ);
	bus_dmamap_sync(tag, map, BUS_DMASYNC_POSTWRITE | BUS_DMASYNC_POSTREAD);

	if (msg->code != RP1_ETH_MBOX_RESP_OK) {
		err = EIO;
	} else {
