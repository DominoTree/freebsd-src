...
rss_print_table(if_ctx *ctx)
{
	...
	width = 2;
	for (max = ifrt.ifrt_nqueues > 0 ? ifrt.ifrt_nqueues - 1 : 0;
	    max >= 10; max /= 10)
		width++;
	for (i = 0; i < ifrt.ifrt_nentries; i++) {
		if (i % 32 == 0)
			printf("\n\t  ");
		printf("%*u", width, ifrt.ifrt_table[i]);
	}
	printf("\n");
}
...
rss_parse_queues(const char *spec, uint16_t *queues, u_int maxqueues)
{
	...
	while ((tok = strsep(&next, ",")) != NULL) {
		...
		/* strtoul() takes a sign or blanks; a queue is digits. */
		...
		if (first > last)
			errx(1, "rsstable: range %lu-%lu is reversed", first,
			    last);
		if (last > UINT16_MAX)
			errx(1, "rsstable: queue %lu is too large", last);
...
setrsstable(if_ctx *ctx, const char *val, int dummy __unused)
{
	...
	if (nqueues == 0) {
		nqueues = MIN(ifrt.ifrt_nqueues, nitems(queues));
		for (i = 0; i < nqueues; i++)
			queues[i] = i;
	} else {
		for (i = 0; i < nqueues; i++) {
			if (queues[i] >= ifrt.ifrt_nqueues)
				errx(1, "rsstable: queue %u out of range "
				    "(0..%u)", queues[i],
				    ifrt.ifrt_nqueues - 1);
		}
	}
